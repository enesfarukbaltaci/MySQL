...
-- bu sorgulama yanlş olur. group by ile tabloda geçen ayn isimleri alrz.
...
select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas),2) as ortalama_yas from personel group by ulke,cinsiyet;



-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, round(avg(yas), 2) as ortalama_yas, count(*) as kisi
from personel
where maas>30000
group by ulke, cinsiyet;
...
select isim, urun_adi, sum(urun_miktari) as tp
from manav
group by isim, urun_adi
having tp >=3
order by tp desc;
...
from manav
group by urun_adi
having max(urun_miktari) <> min(urun_miktari)	-- <> terine != kullanailiriz
order by max(urun_miktari);


select isim,urun_adi,max(urun_miktari) from manav group by urun_adi
